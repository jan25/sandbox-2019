...
class PasswordGenerator(service_pb2_grpc.PasswordGeneratorServicer):
    '''
    ...
    def rotateStr(self, s, by):
        return ''.join([chr((ord(c) + by + 26) % 26) for c in s])

def serve():
    server = grpc.server(concurrent.futures.ThreadPoolExecutor(max_workers=10))
    service_pb2_grpc.add_PasswordGeneratorServicer_to_server(
        service_pb2_grpc.PasswordGeneratorServicer(), server)
